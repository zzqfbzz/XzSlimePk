...
- **grid（铺格）**：从范围的**左上角（最小坐标）**开始，每隔 `windowSize` 铺一块
  **互不重叠**的格子，只统计完整落在范围内的整块（尾部不足一块的丢弃）。
  例：范围区块 0~100、窗口 8 → 第一块是 (0,0)~(7,7)，接着 (8,0)~(15,7)…
  适合：想把范围按固定格子分块统计的场景。
- **sliding（滑动窗口）**：窗口起点**逐区块滑动**（可 `skip` 抽样），
  找到“任意摆放”下的最优窗口位置。适合：想精确知道 8×8（单个玩家可加载的最大区域）
  窗口在这片范围里放哪最合适。sliding 的候选集包含 grid 的所有格子。
